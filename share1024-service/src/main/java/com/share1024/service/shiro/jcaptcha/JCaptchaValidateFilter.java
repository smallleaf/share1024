...
public class JCaptchaValidateFilter extends AccessControlFilter {
	...
	private Logger logger = LoggerFactory.getLogger(JCaptchaValidateFilter.class);
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {
        ...
        String jcaptchaCode = httpServletRequest.getParameter(jcaptchaParam);
        logger.info("==输入验证码为:{}==",jcaptchaCode);
        //3、此时是表单提交，验证验证码是否正确
        return JCaptcha.validateResponse(httpServletRequest, jcaptchaCode);
    }
    @Override
